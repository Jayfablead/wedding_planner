...
class _AddguestlistState extends State<Addguestlist> {
      ...
      if (internet) {
        taskprovider().myvenuetableapi().then((response) async {
          ...
          if (response.statusCode == 200 && myvenuetablemodal?.status == "1") {
            ...
            i++) {
              items.add(test((myvenuetablemodal?.data?[i].name).toString(),(myvenuetablemodal?.data?[i].id).toString()));
            }
  ...
  addguestap() {
    if (_formKey.currentState!.validate() && (selectedItem?.id  != null || selectedItem?.id  != "")) {
